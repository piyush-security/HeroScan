...
from HeroScan.config import config, configurable_keys, configurable_boolean_keys
from HeroScan.io import slugify, e, fformat, cprint, debug, info, warn, error, fail, CommandStreamReader
from HeroScan.plugins import Pattern, PortScan, ServiceScan, Report, HeroScan
from HeroScan.targets import Target, Service
...
if not os.path.exists(config['config_dir']):
	...
	if not os.path.exists(os.path.join(config['config_dir'], 'VERSION-' + VERSION)):
		warn('It looks like the config/plugins in ' + config['config_dir'] + ' are outdated. Please remove the ' + config['config_dir'] + ' directory and re-run HeroScan to rebuild them.')
...
def cancel_all_tasks(signal, frame):
	...
	for target in HeroScan.scanning_targets:
	...
	while True:
		...
		async with target.lock:
			...
			if count > 1:
				...
				info('{bgreen}' + current_time + '{rst} - There is {byellow}One{rst} scan still running against {byellow}' + target.address + '{rst}' + tasks_list)
...
async def get_semaphore(HeroScan):
	semaphore = HeroScan.service_scan_semaphore
	while True:
		# If service scan semaphore is locked, see if we can use port scan semaphore.
		if semaphore.locked():
			if semaphore != HeroScan.port_scan_semaphore: # This will be true unless user sets max_scans == max_port_scans
				...
				if not HeroScan.pending_targets and (config['max_port_scans'] - port_scan_task_count) >= 1: # If no more targets, and we have room, use port scan semaphore.
					if HeroScan.port_scan_semaphore.locked():
						await asyncio.sleep(1)
						continue
					semaphore = HeroScan.port_scan_semaphore
					break
				else: # Do some math to see if we can use the port scan semaphore.
					if (config['max_port_scans'] - (port_scan_task_count + (len(HeroScan.pending_targets) * config['port_scan_plugin_count']))) >= 1:
						if HeroScan.port_scan_semaphore.locked():
							await asyncio.sleep(1)
							continue
						semaphore = HeroScan.port_scan_semaphore
...
async def port_scan(plugin, target):
	...
	async with target.HeroScan.port_scan_semaphore:
...
async def service_scan(plugin, service):
	semaphore = service.target.HeroScan.service_scan_semaphore

	if not config['force_services']:
		semaphore = await get_semaphore(service.target.HeroScan)
	...
	while plugin_pending:
		...
		if plugin.max_global_instances and plugin.max_global_instances > 0:
			async with service.target.HeroScan.lock:
				# Count currently running plugin instances.
				for target in service.target.HeroScan.scanning_targets:
		...
		async with semaphore:
			...
			nmap_extra = service.target.HeroScan.args.nmap
			if service.target.HeroScan.args.nmap_append:
				nmap_extra += ' ' + service.target.HeroScan.args.nmap_append
...
async def generate_report(plugin, targets):
	...
	if not config['force_services']:
		semaphore = await get_semaphore(HeroScan)
...
async def scan_target(target):
	...
	if config['force_services']:
		...
		if services:
			...
			heartbeat.cancel()
			HeroScan.errors = True
			return
	else:
		for plugin in target.HeroScan.plugin_types['port']:
			...
			if config['port_scans'] and plugin.slug in config['port_scans']:
				...
				matching_tags = False
				for tag_group in target.HeroScan.tags:
					if set(tag_group).issubset(plugin_tag_set):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in target.HeroScan.excluded_tags:
					if set(tag_group).issubset(plugin_tag_set):
						excluded_tags = True
						break
	...
	async with HeroScan.lock:
		HeroScan.scanning_targets.append(target)

	start_time = time.time()
	info('Enumerating Target   {byellow}' + target.address + '{rst}')
	...
	while pending:
		...
		for service in services:
			...
			nmap_extra = target.HeroScan.args.nmap
			if target.HeroScan.args.nmap_append:
				nmap_extra += ' ' + target.HeroScan.args.nmap_append
			...
			for plugin in target.HeroScan.plugin_types['service']:
			...
			if not service_match:
				...
				if service.name not in config['service_exceptions'] and service.full_tag() not in target.HeroScan.missing_services:
					target.HeroScan.missing_services.append(service.full_tag())

	for plugin in target.HeroScan.plugin_types['report']:
	...
	if timed_out:
		...
		warn('{byellow}Enumerating Target   ' + target.address + ' took longer than the specified target period (' + str(config['target_timeout']) + ' min). Cancelling scans and moving to next target.{rst}')
	else:
		info('Finished Enumerating Target   {byellow}' + target.address + '{rst} in ' + elapsed_time)

	async with HeroScan.lock:
		HeroScan.completed_targets.append(target)
		HeroScan.scanning_targets.remove(target)
...
async def run():
	...
	parser.add_argument('-c', '--config', action='store', type=str, default=config_file, dest='config_file', help='Location of HeroScan\'s config file. Default: %(default)s')
	parser.add_argument('-g', '--global-file', action='store', type=str, help='Location of HeroScan\'s global file. Default: %(default)s')
	...
	parser.add_argument('--timeout', action='store', type=int, help='Specifies the maximum amount of time in minutes that HeroScan should run for. Default: %(default)s')
	...
	parser.add_argument('--proxychains', action='store_true', help='Use if you are running HeroScan via proxychains. Default: %(default)s')
	...
	parser.add_argument('--accessible', action='store_true', help='Attempts to make HeroScan output more accessible to screenreaders. Default: %(default)s')
	parser.add_argument('-v', '--verbose', action='count', help='Enable verbose output. Repeat for more verbosity.')
	parser.add_argument('--version', action='store_true', help='Prints the HeroScan version and exits.')
	parser.error = lambda s: fail(s[0].upper() + s[1:])
	args, unknown = parser.parse_known_args()

	errors = False

	HeroScan.argparse = parser

	if args.version:
		print('HeroScan v' + VERSION)
		sys.exit(0)
	...
	if not args.config_file:
		unknown_help()
		fail('Error: Could not find config.toml in the current directory or ~/.config/HeroScan.')
	...
	if not config['plugins_dir']:
		unknown_help()
		fail('Error: Could not find plugins directory in the current directory or ~/.config/HeroScan.')
	...
	for plugins_dir in plugins_dirs:
		for plugin_file in sorted(os.listdir(plugins_dir)):
			if not plugin_file.startswith('_') and plugin_file.endswith('.py'):
				...
				try:
					spec = importlib.util.spec_from_file_location("HeroScan." + filename[:-3], os.path.join(dirname, filename))
					plugin = importlib.util.module_from_spec(spec)
					spec.loader.exec_module(plugin)

					clsmembers = inspect.getmembers(plugin, predicate=inspect.isclass)
					for (_, c) in clsmembers:
						if c.__module__ in ['HeroScan.plugins', 'HeroScan.targets']:
							continue
	...
	for plugin in HeroScan.plugins.values():
		if plugin.slug in HeroScan.taglist:
			unknown_help()
	...
	if len(HeroScan.plugin_types['port']) == 0:
		unknown_help()
		fail('Error: There are no valid PortScan plugins in the plugins directory "' + config['plugins_dir'] + '".')

	# Sort plugins by priority.
	HeroScan.plugin_types['port'].sort(key=lambda x: x.priority)
	HeroScan.plugin_types['service'].sort(key=lambda x: x.priority)
	HeroScan.plugin_types['report'].sort(key=lambda x: x.priority)

	if not config['global_file']:
		unknown_help()
		fail('Error: Could not find global.toml in the current directory or ~/.config/HeroScan.')
						...
						if 'pattern' in pattern:
							try:
								...
								if 'description' in pattern:
									HeroScan.patterns.append(Pattern(compiled, description=pattern['description']))
								else:
									HeroScan.patterns.append(Pattern(compiled))
							except re.error:
								unknown_help()
		...
		if key == 'global' and isinstance(val, dict): # Process global plugin options.
			for gkey, gval in config_toml['global'].items():
				if isinstance(gval, bool):
					for action in HeroScan.argparse._actions:
						if action.dest == 'global.' + slugify(gkey).replace('-', '_'):
							if action.const is True:
								action.__setattr__('default', gval)
							break
				else:
					if HeroScan.argparse.get_default('global.' + slugify(gkey).replace('-', '_')):
						HeroScan.argparse.set_defaults(**{'global.' + slugify(gkey).replace('-', '_'): gval})
		elif isinstance(val, dict): # Process potential plugin arguments.
			for pkey, pval in config_toml[key].items():
				if HeroScan.argparse.get_default(slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_')) is not None:
					for action in HeroScan.argparse._actions:
						if action.dest == slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'):
							if action.const and pval != action.const:
								...
								errors = True
							break
					HeroScan.argparse.set_defaults(**{slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'): pval})
		else: # Process potential other options.
			key = key.replace('-', '_')
			if key in configurable_keys:
				other_options.append(key)
				config[key] = val
				HeroScan.argparse.set_defaults(**{key: val})
	...
	if args.list:
		...
		if type in ['plugin', 'plugins', 'port', 'ports', 'portscan', 'portscans']:
			for p in HeroScan.plugin_types['port']:
	...
	if config['max_plugin_target_instances']:
		for plugin_instance in config['max_plugin_target_instances']:
			...
			if len(plugin_instance) == 2:
				if plugin_instance[0] not in HeroScan.plugins:
	...
	for slug, plugin in HeroScan.plugins.items():
	...
	if not errors:
		if config['force_services']:
			HeroScan.service_scan_semaphore = asyncio.Semaphore(config['max_scans'])
		else:
			HeroScan.port_scan_semaphore = asyncio.Semaphore(config['max_port_scans'])
			...
			if config['max_scans'] == config['max_port_scans']:
				HeroScan.service_scan_semaphore = HeroScan.port_scan_semaphore
			else:
				HeroScan.service_scan_semaphore = asyncio.Semaphore(config['max_scans'] - config['max_port_scans'])
	...
	[HeroScan.tags.append(t) for t in tags if t not in HeroScan.tags]
	...
	if args.exclude_tags != '':
		...
		# Remove duplicate lists from list.
		[HeroScan.excluded_tags.append(t) for t in excluded_tags if t not in HeroScan.excluded_tags]
	...
	for target in raw_targets:
		try:
			...
			found = False
			for t in HeroScan.pending_targets:
				if t.address == ip_str:
					found = True
					break
			...
			if isinstance(ip, ipaddress.IPv4Address):
				HeroScan.pending_targets.append(Target(ip_str, ip_str, 'IPv4', 'ip', HeroScan))
			elif isinstance(ip, ipaddress.IPv6Address):
				HeroScan.pending_targets.append(Target(ip_str, ip_str, 'IPv6', 'ip', HeroScan))
			else:
			...
			try:
				...
				try:
					...
					HeroScan.pending_targets.append(Target(target, ip, 'IPv4', 'hostname', HeroScan))
					...
					try:
						...
						HeroScan.pending_targets.append(Target(target, ip, 'IPv6', 'hostname', HeroScan))
					except socket.gaierror:
	...
	if not args.disable_sanity_checks and unresolvable_targets == True:
		error('HeroScan will not run if any targets are invalid / unresolvable. To override this, re-run with the --disable-sanity-checks option.')
		errors = True

	if len(HeroScan.pending_targets) == 0:
		error('You must specify at least one target to scan!')
		errors = True

	if config['single_target'] and len(HeroScan.pending_targets) != 1:
		error('You cannot provide more than one target when scanning in single-target mode.')
		errors = True

	if not args.disable_sanity_checks and len(HeroScan.pending_targets) > 256:
		error('A total of ' + str(len(HeroScan.pending_targets)) + ' targets would be scanned. If this is correct, re-run with the --disable-sanity-checks option to suppress this check.')
		errors = True

	if not config['force_services']:
		port_scan_plugin_count = 0
		for plugin in HeroScan.plugin_types['port']:
			if config['port_scans'] and plugin.slug in config['port_scans']:
				matching_tags = True
				excluded_tags = False
			else:
				matching_tags = False
				for tag_group in HeroScan.tags:
					if set(tag_group).issubset(set(plugin.tags)):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in HeroScan.excluded_tags:
					if set(tag_group).issubset(set(plugin.tags)):
						excluded_tags = True
						break
	...
	pending = []
	i = 0
	while HeroScan.pending_targets:
		pending.append(asyncio.create_task(scan_target(HeroScan.pending_targets.pop(0))))
		i+=1
	...
	while pending:
		...
		# If something failed in scan_target, HeroScan.errors will be true.
		if HeroScan.errors:
		...
		for task in done:
			if HeroScan.pending_targets:
				pending.add(asyncio.create_task(scan_target(HeroScan.pending_targets.pop(0))))
			if task in pending:
				pending.remove(task)

		port_scan_task_count = 0
		for targ in HeroScan.scanning_targets:
	...
	if len(HeroScan.completed_targets) > 1:
		for plugin in HeroScan.plugin_types['report']:
			plugin_tag_set = set(plugin.tags)
			...
			if matching_tags and not excluded_tags:
				pending.add(asyncio.create_task(generate_report(plugin, HeroScan.completed_targets)))
	...
	if timed_out:
		...
		elapsed_time = calculate_elapsed_time(start_time)
		warn('{byellow}HeroScan took longer than the specified timeout period (' + str(config['timeout']) + ' min). Cancelling all scans and exiting.{rst}')
	else:
	...
	if HeroScan.missing_services:
		warn('{byellow}HeroScan identified the following services, but could not match them to any plugins based on the service name. Please report these to P1YUSH: ' + ', '.join(HeroScan.missing_services) + '{rst}')
