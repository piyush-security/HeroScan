...
class Enum4Linux(ServiceScan):
	...
	def check(self):
		...
		if tool == 'enum4linux' and which('enum4linux') is None:
			self.error('The enum4linux program could not be found.(On your terminal, run: sudo apt install enum4linux)')
			return False
		elif tool == 'enum4linux-ng' and which('enum4linux-ng') is None:
			self.error('The enum4linux-ng program could not be found. (https://github.com/cddmp/enum4linux-ng)')
			return False
