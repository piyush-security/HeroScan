from HeroScan.plugins import ServiceScan
...
class DirBuster(ServiceScan):
	...
	def configure(self):
		...
		self.add_option('threads', default=30, help='The number of threads to use when directory busting. Default: %(default)s')
	...
	def check(self):
		...
		if tool == 'feroxbuster' and which('feroxbuster') is None:
			self.error('The feroxbuster program could not be found. (On your terminal, run: sudo apt install feroxbuster)')
			return False
		elif tool == 'gobuster' and which('gobuster') is None:
			self.error('The gobuster program could not be found. (On your terminal, run: sudo apt install gobuster)')
			return False
		elif tool == 'dirsearch' and which('dirsearch') is None:
			self.error('The dirsearch program could not be found. (On your terminal, run: sudo apt install dirsearch)')
			return False
		elif tool == 'ffuf' and which('ffuf') is None:
			self.error('The ffuf program could not be found. (On your terminal, run: sudo apt install ffuf)')
			return False
		elif tool == 'dirb' and which('dirb') is None:
			self.error('The dirb program could not be found. (On your terminal, run: sudo apt install dirb)')
			return False
