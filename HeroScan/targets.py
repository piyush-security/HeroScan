...
from HeroScan.io import e, info, warn, error

class Target:

	def __init__(self, address, ip, ipversion, type, HeroScan):
		self.address = address
		self.ip = ip
		self.ipversion = ipversion
		self.type = type
		self.HeroScan = HeroScan
		self.basedir = ''
		self.reportdir = ''
		self.scandir = ''
	...
	def extract_service(self, line, regex=None):
		return self.HeroScan.extract_service(line, regex)

	async def extract_services(self, stream, regex=None):
		return await self.HeroScan.extract_services(stream, regex)
	...
	async def execute(self, cmd, blocking=True, outfile=None, errfile=None, future_outfile=None):
		...
		process, stdout, stderr = await target.HeroScan.execute(cmd, target, tag, patterns=plugin.patterns, outfile=outfile, errfile=errfile)
